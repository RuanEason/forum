      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Link href="/" className="flex items-center space-x-2 group">
            ...
            <span className="text-xl font-bold text-gray-900 tracking-tight">同学论坛</span>
          </Link>
          ...
          <nav className="flex items-center space-x-4">
            {status === "authenticated" ? (
              <div className="flex items-center space-x-3 sm:space-x-4">
                ...
                <Link href={`/user/${session.user.id}`} className="flex items-center space-x-2 text-gray-700 hover:text-gray-900 group">
                  <Avatar src={session.user.avatar} name={session.user.name} size="sm" className="ring-2 ring-transparent group-hover:ring-indigo-100 transition-all" />
                ...
                <button
                  onClick={() => signOut()}
                  className="text-sm font-medium text-gray-500 hover:text-red-600 transition-colors px-3 py-1.5 rounded-md hover:bg-red-50"
                >
                  退出
                </button>
              </div>

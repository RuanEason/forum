...
export async function POST(request: Request) {
  try {
    ...
    const backgroundVideoAssetId =
      typeof body.backgroundVideoAssetId === "string" ? body.backgroundVideoAssetId.trim() : "";
    ...
    if (!backgroundVideoAssetId || !objectKey) {
      return NextResponse.json(
        { error: "backgroundVideoAssetId and objectKey are required" },
        { status: 400 },
      );
    }

    if (!objectKey.startsWith(constraints.rawPrefix)) {
      return NextResponse.json({ error: "objectKey is outside backgrounds prefix" }, { status: 400 });
    }

    const videoAsset = await prisma.videoAsset.findUnique({
      where: { id: backgroundVideoAssetId },
      select: {
        id: true,
        ownerId: true,
        rawObjectKey: true,
    ...
    if (!["UPLOADING", "FAILED"].includes(videoAsset.status)) {
      return NextResponse.json(
        { error: `invalid background video status: ${videoAsset.status}` },
        { status: 400 },
      );
    }
    ...
    if (
      ...
      && !VIDEO_ALLOWED_MIME_TYPES.includes(
        normalizedHeadMimeType as (typeof VIDEO_ALLOWED_MIME_TYPES)[number],
      )
    ) {
      return NextResponse.json(
    ...
    console.error("Background video commit error:", error);
    return NextResponse.json(

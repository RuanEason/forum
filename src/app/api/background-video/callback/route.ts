...
function collectPrimitiveValues(
  source: unknown,
  bucket: Array<{ key: string; value: Primitive }>,
  parentKey = "",
) {
...
function selectBestTranscodedVideoObjectKey(values: string[], sourceKey: string): string | undefined {
  const objectKeys = values
    .map((value) => value.trim())
    .filter((value) => /\.(mp4|mov|avi|webm)(?:$|\?)/i.test(value))
  ...
  const mp4 = uniqueObjectKeys.find((candidate) => /\.mp4(?:$|\?)/i.test(candidate));
  if (mp4) {
    return mp4;
  }

  return uniqueObjectKeys[0];
}
...
function extractCallbackPayload(body: unknown) {
  ...
  const rawPrefixSource = sourceKey
    || stringValues.find((value) => value.includes("backgrounds/"))
    || stringValues.find((value) => value.includes("background/"))
    || "";

  const normalizedSourceKey = normalizeObjectKey(rawPrefixSource);
  const explicitVideoCandidate = firstStringFromSources(sources, [
  ...
  const transcodedVideoObjectKey = selectBestTranscodedVideoObjectKey(
    [
      ...(explicitVideoCandidate ? [explicitVideoCandidate] : []),
      ...stringValues,
    ],
    normalizedSourceKey,
  );
...
export async function POST(request: Request) {
  try {
    ...
    if (!parsed.normalizedSourceKey.startsWith(constraints.rawPrefix)) {
      return NextResponse.json(
        { error: "Source object key is outside backgrounds prefix" },
        { status: 400 },
      );
    }
    ...
    if (parsed.success && !parsed.transcodedVideoObjectKey) {
      if (parsed.eventName?.toLowerCase() === "taskfinish") {
        return NextResponse.json({
          ok: true,
          message: "TaskFinish callback without final video output ignored",
        });
      }
      ...
      await prisma.videoAsset.update({
        ...
        data: {
          ...
          errorMessage: "Workflow succeeded but no transcoded video output path was found",
        },
      });

      return NextResponse.json(
        { ok: false, error: "Missing transcoded video output in callback payload" },
        { status: 400 },
      );
    }
    ...
    if (parsed.success && parsed.transcodedVideoObjectKey) {
      await prisma.videoAsset.update({
        ...
        data: {
          status: "READY",
          workflowRunId: parsed.workflowRunId || undefined,
          coverObjectKey: parsed.transcodedVideoObjectKey,
          coverUrl: buildVideoCdnUrl(parsed.transcodedVideoObjectKey),
      ...
      return NextResponse.json({
        ok: true,
        status: "READY",
        videoUrl: buildVideoCdnUrl(parsed.transcodedVideoObjectKey),
      });
    }
    ...
    console.error("Background video callback error:", error);
    return NextResponse.json(
